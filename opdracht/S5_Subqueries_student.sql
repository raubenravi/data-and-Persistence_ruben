...
SELECT cursist 
FROM inschrijvingen
WHERE cursus = 'JAV' AND cursist IN (select cursist 
			  FROM inschrijvingen
			  WHERE cursus = 'XML'  );
...
SELECT naam
FROM medewerkers onderdaan
WHERE mnr NOT IN (SELECT  chef
			 FROM medewerkers
		     WHERE onderdaan.mnr = chef);
			 --per werk nemer dus kijken als hij leiding gevende heeft
...
from uitvoeringen u
WHERE date_part('year',  begindatum ) = 2020 and cursus in (SELECT code
														   FROM cursussen
														   WHERE type = 'BLD') ;
...
(SELECT count(*) as aantal_inschrijvingen
 FROM inschrijvingen
 WHERE u.cursus = cursus and u.begindatum = begindatum
)
from uitvoeringen u
...
SELECT DISTINCT mdw.voorl, mdw.naam
FROM medewerkers mdw
JOIN uitvoeringen u on u.docent = mdw.mnr
where mdw.chef in (
	SELECT DISTINCT cursist
	FROM medewerkers cursist
	JOIN inschrijvingen alg_cursissten on alg_cursissten.cursus in (SELECT code 
	  FROM cursussen
	  WHERE type = 'ALG' )
	WHERE alg_cursissten.cursus = u.cursus and alg_cursissten.begindatum = u.begindatum	
);
