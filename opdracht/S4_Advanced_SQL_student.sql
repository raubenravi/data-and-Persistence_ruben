...
WHERE  gbdatum < TO_DATE('1980-01-01', 'yyyy-mm-dd') and (functie = 'VERKOPER' OR functie  = 'TRAINER');
...
where naam ILIKE '% %';	 --mits er geen onodige spaties zijn op het einde of beging, dus alleen bij integere data
--where naam ILIKE 'De %' OR naam ILIKE 'van %' OR naam ILIKE 'den %' OR naam ILIKE 'der %';
...
select (  max(gbdatum) - min(gbdatum) ) / 365  as verschil, avg( CURRENT_DATE - gbdatum) / 365 as gemiddeld  FROM medewerkers;
...
select count(*) as aantal_medewerkers, round(sum(comm) / count(*))  as commissie_medewerkers, round( avg(comm)    )    as commissie_verkopers FROM medewerkers;
